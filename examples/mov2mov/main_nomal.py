...
def main(
    ...
    # ax1.set_ylabel("Skip Probability", color="tab:red", fontsize=14)
    # ax1.plot(np.arange(len(skip_probs)), skip_probs, color="tab:red", label="Skip Probability", linewidth=2)
    ...
    # ax2.plot(np.arange(len(utilization_rec)), utilization_rec, color="tab:blue", label="GPU Utilization with filter", linewidth=2)
    # ax2.plot(np.arange(len(utilization_wo_filter)), utilization_wo_filter, color="tab:orange", label="GPU Utilization without filter", linewidth=2)
    # ax2.tick_params(axis="y", labelcolor="tab:blue")
    # ax2.legend(loc="upper left")
    # # ax2.set_ylabel("GPU Power Usage (W)", color="tab:blue", fontsize=14)
    # # ax2.plot(np.arange(len(utilization_rec)), gpu_power_usages, color="tab:blue", label="GPU Power Usage with filter", linewidth=2)
    # # ax2.plot(
    # #     np.arange(len(utilization_wo_filter)), gpu_power_usages_wo_filter, color="tab:orange", label="GPU Power Usage without filter", linewidth=2
    # # )
    # # ax2.tick_params(axis="y", labelcolor="tab:blue")
    # # ax2.legend(loc="upper left")

    # # Title and layout adjustments
    # plt.title("GPU Processor Utilization and Skip Probability", fontsize=16)
    # fig.tight_layout()
    ...
    # df = pd.DataFrame(
    #     {
    #         "skip_prob": skip_probs,
    #         "utilization": utilization_rec,
    #         "utilization_wo_filter": utilization_wo_filter,
    #         "gpu_power_usages": gpu_power_usages,
    #         "gpu_power_usages_wo_filter": gpu_power_usages_wo_filter,
    #     }
    # )
